...
@prompts [
%{x: false, prompt: "A weird thing for the letters in your alphabet soup to suddenly spell out"},
%{x: false, prompt: "A great prank to play on a pizza delivery guy"},
%{x: false, prompt: "The most surprising thing you could find in the glove box of a rental car"},
...
%{x: false, prompt: "Little-known fact: In a lifetime, the average person will <BLANK> over 1,000 times while sleeping"},
...
%{x: false, prompt: "Few remember Michelangelo\u2019s <i>Mona Lisa 2<\/i> which was a painting of <BLANK>"},
...
%{x: false, prompt: "What to do when a really tall person sits in front of you at the movie theater"},
...
%{x: false, prompt: "What ruined Hannibal \u201cThe Cannibal\u201d Lecter\u2019s credit score?"},
%{x: false, prompt: "What the lamest Transformer would morph into"},
%{x: false, prompt: "You would gladly give money to someone on the street if they asked \u201cCan you spare some change so I can <BLANK>?\u201d"},
%{x: false, prompt: "SPOILER ALERT: The big plot twist in <i>The Sisterhood of the Traveling Pants 7<\/i> is that the pants <BLANK>"},
...
%{x: false, prompt: "The title of a National Public Radio show that would put you to sleep the quickest"},
...
%{x: false, prompt: "If Cap\u2019n Crunch ever gets court-martialed, it\u2019ll probably be because he..."},
...
%{x: false, prompt: "Bob the Builder probably wouldn\u2019t be as popular with children if he were Bob the <BLANK>"},
%{x: false, prompt: "The worst thing to do when a bear is right next to you"},
%{x: false, prompt: "Unlike \u201cMaverick\u201d or \u201cIceman,\u201d a really bad Air Force fighter pilot name would be <BLANK>"},
%{x: false, prompt: "It would\u2019ve been a much different movie if instead of \u201cPhone home,\u201d E.T. kept saying, \u201c<BLANK>\u201d"},
...
%{x: false, prompt: "A mystery that Sherlock Holmes could never solve: The Case of the <BLANK>"},
...
%{x: false, prompt: "You know you\u2019re comfortable in a relationship when you ask your significant other to <BLANK>"},
...
%{x: false, prompt: "Come up with a <i>TMZ<\/i> celebrity headline from five years in the future"},
%{x: false, prompt: "What the roller coaster attendant is actually saying during his mumbled preamble before the ride"},
...
%{x: false, prompt: "The creepiest thing to whisper in somebody\u2019s ear as you\u2019re hugging them"},
...
%{x: false, prompt: "What the Queen\u2019s Guard is secretly thinking as they just stand there"},
%{x: false, prompt: "The worst part about having a mustache"},
%{x: true, prompt: "An awkward thing to hear from the person pooping in the bathroom stall next to you"},
%{x: false, prompt: "A quick way to save money on grocery bills"},
%{x: false, prompt: "A good sign you\u2019re never going to be a professional football player"},
...
%{x: false, prompt: "The lesser-known sequel to <i>Old Yeller<\/i>: <i>Old Yeller 2: <BLANK><\/i>"},
...
%{x: false, prompt: "The only thing worse than standing in a really long line is standing in a really long line for <BLANK>"},
...
%{x: false, prompt: "A rejected tourism slogan for Des Moines, Iowa: \u201cHome of the <BLANK>\u201d"},
%{x: false, prompt: "A forgotten book in the classic Harry Potter series: <i>Harry Potter and the <BLANK><\/i>"},
...
%{x: false, prompt: "The big, crazy twist at the end of the next M. Night Shamalayan movie: He was <BLANK> the whole time!"},
%{x: false, prompt: "Most people know it as The Big Apple, but a lesser-known nickname for New York is The Big <BLANK>"},
%{x: false, prompt: "The next best thing to chew when you\u2019re out of gum"},
%{x: false, prompt: "You know you\u2019re in a very weird fast food restaurant when the cashier asks, \u201cDo you want <BLANK> with that?\u201d"},
%{x: false, prompt: "It\u2019s not the heat. It\u2019s not the humidity. It\u2019s the <BLANK>"},
%{x: false, prompt: "It\u2019s incredibly rude to <BLANK> with your mouth open"},
%{x: false, prompt: "You never have a <BLANK> when you need one"},
%{x: false, prompt: "<i>The Empire Strikes Back<\/i> would\u2019ve been ruined if Darth Vader said \u201cLuke, I am <BLANK>\u201d"},
...
%{x: false, prompt: "Worse than global warming, the real threat to humanity is global <BLANK>"},
...
%{x: false, prompt: "If evolution is true, then why hasn\u2019t <BLANK> evolved into <BLANK>?"},
...
%{x: false, prompt: "A little-known lyric in the original draft of the \u201cStar-Spangled Banner\u201d"},
%{x: false, prompt: "The best thing to shoot out of a cannon"},
%{x: false, prompt: "The winners on <i>The Bachelor<\/i> get a rose. The losers should get <BLANK>"},
%{x: false, prompt: "From the creators of \u201cWhack-a-Mole\u201d comes the new game \u201c<BLANK>-a-<BLANK>\u201d"},
%{x: false, prompt: "The title of a never-released Jimmy Buffett song"},
%{x: false, prompt: "The worst thing to hear from your GPS: \u201cIn two miles, <BLANK>\u201d"},
...
%{x: false, prompt: "America\u2019s energy crisis would be over if we made cars that ran on <BLANK>"},
%{x: false, prompt: "Something it\u2019d be fun to watch ride an escalator "},
%{x: false, prompt: "A high school superlative you don\u2019t want to win: Most Likely To <BLANK>"},
%{x: false, prompt: "A rejected title for <i>Moby Dick<\/i>"},
%{x: false, prompt: "Something you do not want to find under your hotel bed"},
%{x: false, prompt: "You know your doctor has gone insane when he tells you to make sure you <BLANK> at least once a day"},
...
%{x: false, prompt: "A video sure to get over 150 million views on YouTube would be \u201cChickens <BLANK>\u201d"},
...
%{x: false, prompt: "What Waldo from \u201cWhere\u2019s Waldo?\u201d says to himself in the mirror"},
...
%{x: false, prompt: "What should we do with all of that plastic that won\u2019t disintegrate?"},
...
%{x: false, prompt: "The lesser-known other way to find the Wizard of Oz: Follow the <BLANK> Road"},
%{x: false, prompt: "Forget coffee. Don\u2019t talk to me until I\u2019ve had my <BLANK>"},
%{x: false, prompt: "Odd new shampoo instructions: \u201cLather, Rinse, <BLANK>, Repeat.\u201d"},
...
%{x: false, prompt: "What is the Abraham Lincoln statue thinking while he\u2019s sitting there in the Lincoln Memorial?"},
%{x: false, prompt: "Instead of \u201cCheese!\u201d the worst family photographer would tell you to say, \u201c<BLANK>!\u201d"},
%{x: false, prompt: "The title of a college admission essay that would definitely get rejected"},
%{x: false, prompt: "Something Big Bird will confess on his deathbed"},
%{x: false, prompt: "What you would expect Justin Bieber\u2019s line of fragrances to smell like"},
...
%{x: false, prompt: "In the future, moviegoers will flock to see <i>Jurassic Park 10: <BLANK><\/i>"},
%{x: false, prompt: "The worst way to unclog a toilet"},
%{x: false, prompt: "Something that\u2019s been hiding in the background in every episode of <i>Friends<\/i>"},
...
%{x: false, prompt: "If the groundhog \u201ckind of\u201d sees his shadow, it\u2019s six weeks of <BLANK>"},
...
%{x: false, prompt: "What you don\u2019t want to hear from the passenger next to you at the start of a 20-hour flight"},
...
%{x: false, prompt: "The worst combination of two actors that could possibly star in the next season of <i>True Detective<\/i> together"},
%{x: false, prompt: "It\u2019s disappointing to put together a 1,000 piece puzzle and realize it\u2019s just a picture of <BLANK>"},
...
%{x: false, prompt: "A double rainbow doesn\u2019t have gold at the end of it. Instead, it has <BLANK>"},
%{x: false, prompt: "The best shirt to wear next to somebody who\u2019s wearing an \u201cI\u2019m with stupid\u201d T-shirt"},
%{x: false, prompt: "The worst thing a plastic surgeon could say after he botched your surgery: \u201cI\u2019m sorry, I accidentally <BLANK>\u201d"},
...
%{x: false, prompt: "A weird thing to hear from your doctor: \u201cI\u2019m afraid you have <BLANK>\u201d"},
%{x: false, prompt: "In a shocking poll, it was discovered that three out of four Americans <BLANK>"},
%{x: false, prompt: "The most common thing you\u2019d hear if you could read people\u2019s thoughts"},
...
%{x: false, prompt: "The worst breakfast: pancakes shaped like <BLANK>"},
%{x: false, prompt: "A sign that you\u2019re a bad teacher"},
%{x: false, prompt: "If a genie gives you three wishes, the best things to wish for are: 1) a billion dollars, 2) eternal life, and 3) <BLANK>"},
%{x: false, prompt: "The worst charity: Save the <BLANK>"},
%{x: false, prompt: "Little-known fact: An unaired episode of <i>The Brady Bunch<\/i> had the family dealing with <BLANK>"},
...
%{x: false, prompt: "Invent a new word for the toilet that sounds like it\u2019s from Shakespeare"},
...
%{x: false, prompt: "In the future, scientists discover that the secret to eternal youth is <BLANK>"},
...
%{x: false, prompt: "A really stupid idea for a phone app (that would still make you millions of dollars)"},
...
%{x: false, prompt: "If you don\u2019t have extra money, an odd thing to use as a tip for your waiter"},
...
%{x: false, prompt: "Like \u201cdinger,\u201d \u201cgrand salami,\u201d and \u201cjack,\u201d come up with a new slang term for a home run in baseball"},
...
%{x: false, prompt: "The years 2011 to 2020 will be remembered as \u201cThe <BLANK> Age\u201d"},
%{x: false, prompt: "An inventive way to get rid of head lice"},
%{x: false, prompt: "Little-known fact: the scariest animal in the world is the <BLANK> cobra"},
...
%{x: false, prompt: "A ridiculous government agency that no one knows about: The Department of <BLANK>"},
%{x: false, prompt: "The best thing about being thrown into a volcano"},
%{x: false, prompt: "The world\u2019s most boring video game"},
%{x: false, prompt: "New requirement at amusement parks: \u201cYou must be this <BLANK> to ride\""},
...
%{x: false, prompt: "Something that is probably on Nicolas Cage\u2019s \u201cTo Do\u201d list"},
%{x: false, prompt: "The newest health food: <BLANK> juice"},
%{x: false, prompt: "HR would probably get the most complaints on \u201cBring your <BLANK> to work\u201d day"},
...
%{x: false, prompt: "What the boogeyman is afraid of"},%{x: false, prompt: "A bad name for a hospital"},
...
%{x: false, prompt: "After the Heimlich Maneuver, the second-best way to stop someone from choking"},
%{x: false, prompt: "A rejected safety technique for when you catch fire was to \u201cstop, drop, and <BLANK>\u201d"},
%{x: false, prompt: "The difference between Grade A beef and Grade B beef"},
%{x: false, prompt: "The weirdest room you could see in an airport would be one specifically designated for <BLANK>"},
%{x: false, prompt: "If you really want to impress the Olympic diving judges, try a dive that involves <BLANK>"},
...
%{x: false, prompt: "A rejected tagline for <i>Star Trek<\/i> instead of \u201cSpace: the final frontier\u201d was \u201cSpace: <BLANK>\u201d"},
...
%{x: false, prompt: "The name of the music playlist that will definitely put an end to the party"},
...
%{x: false, prompt: "Kennedy\u2019s original speech said, \u201cAsk not what your country can do for you, ask <BLANK>\u201d"},
...
%{x: false, prompt: "In the future, RoboCop actually retires from the police force and becomes Robo-<BLANK>"},
...
%{x: false, prompt: "A peculiar thing to see in a Help Wanted ad would be \u201cHelp Wanted: <BLANK>\u201d"},
...
%{x: false, prompt: "Come up with a name for a generic brand of hot dogs that you probably shouldn\u2019t buy"},
...
%{x: false, prompt: "Something a weatherman might yell if he completely snapped during the weather forecast"},
...
%{x: false, prompt: "A clever thing for James Bond to say as he throws someone out of an airplane"},
...
%{x: false, prompt: "The Four Horsemen wouldn\u2019t be as scary if they were the Four <BLANK>men of the Apocalypse"},
%{x: false, prompt: "It\u2019s illegal to yell \u201cFire!\u201d in a crowded theater, but it should also be illegal to yell, \u201c<BLANK>!\u201d"},
...
%{x: false, prompt: "Sometimes John Travolta wildly mispronounces names. How might he wildly mispronounce his own name?"},
...
%{x: false, prompt: "A possible entry in Gary Busey\u2019s dream journal: \u201cTonight I dreamed <BLANK>\u201d"},
...
%{x: false, prompt: "Little-known fact: Over the course of a lifetime, an average person accidentally eats ten <BLANK>"},
...
%{x: false, prompt: "It would be awesome to win <i>Jeopardy<\/i> with the phrase, \u201cWhat is <BLANK>, Alex?\u201d"},
...
%{x: false, prompt: "The sound a tree actually makes when it falls and no one is around to hear it"},
...
%{x: false, prompt: "The perfect meal would be a <BLANK> stuffed in a <BLANK> stuffed in a <BLANK>"},
...
%{x: false, prompt: "A Girl Scouts cookie name that got rejected because it was just too ridiculous-sounding"},
...
%{x: false, prompt: "A terrible sportscaster catchphrase for when somebody dunks a basketball"},
...
%{x: false, prompt: "Something you\u2019d yell to heckle the performing dolphins at Sea World"},
...
%{x: false, prompt: "Tomorrow\u2019s news headline: \u201cScientists Are Shocked to Discover That <BLANK>\u201d"},
...
%{x: false, prompt: "It would be scary to read on a food package, \u201cMay contain trace elements of <BLANK>.\u201d"},
%{x: false, prompt: "A just-so-crazy-it\u2019s-brilliant business idea to pitch on <i>Shark Tank<\/i>"},
...
%{x: false, prompt: "If you\u2019d never heard the term \u201cgreat white shark,\u201d what might you call it when you saw one for the first time?"},
...
%{x: false, prompt: "Make up a word for the watery substances that come out of a ketchup bottle when you first squeeze it"},
...
%{x: false, prompt: "Something you probably shouldn\u2019t bring on a trip across the Sahara desert"},
...
%{x: false, prompt: "Something it\u2019s not a good idea to put in the overhead bin on an airplane"},
%{x: false, prompt: "A weird thing for a bank robber to demand in a hostage situation"},
%{x: false, prompt: "Something they will probably never make a series of commemorative stamps for"},
...
%{x: false, prompt: "Something that would make a creepy replacement for the horses on a merry-go-round"},
...
%{x: false, prompt: "Like Plutonium or Einsteinium, what would you name the next Periodic Table element they discover?"},
%{x: false, prompt: "A bad name for a pirate"},
%{x: false, prompt: "Something fun to scream when you win in a game of bingo, other than \u201cBingo!\u201d"},
...
%{x: false, prompt: "You never know when you\u2019re going to need insurance. You could wake up tomorrow and <BLANK>"},
...
%{x: false, prompt: "The first and second rules of Fight Club are \u201cDon\u2019t talk about Fight Club,\u201d but what\u2019s the 387th rule of Fight Club?"},
%{x: false, prompt: "A terrible food truck would be one that goes around selling only <BLANK>"},
%{x: false, prompt: "A reason to get into a fist fight with a koala bear "},
%{x: false, prompt: "Little-known fact: the fourth Wise Man gave baby Jesus the worst gift of all: <BLANK> "},
...
%{x: false, prompt: "\u201cThis just in! A <BLANK> has won the election and will become the new governor of Texas.\u201d "},
...
%{x: false, prompt: "People wouldn\u2019t respect He-Man as much if, to gain his power, he held up his sword and shouted \u201c____________________\u201d"},
...
%{x: false, prompt: "It\u2019s bad to be buried alive. It\u2019s worse to be buried alive with <BLANK>."},
...
%{x: false, prompt: "A rejected title for <i>The Good, The Bad and the Ugly<\/i> was <i>The Good, the Bad and the <BLANK><\/i>"},
%{x: false, prompt: "Little-known fact: The government allows peanut butter to contain up to 10% <BLANK>"},
...
%{x: false, prompt: "A faster way to get home from the Land of Oz is to click your heels three times and say <BLANK>."},
...
%{x: false, prompt: "Finish this sentence: \u201cWhen I\u2019m rich, my mansion will have a room called The <BLANK> Room.\u201d"},
%{x: false, prompt: "The best question to ask God when you meet him"},
%{x: false, prompt: "A fun trick to play on your doctor"},
%{x: false, prompt: "A bad place for your rocket ship to crash would be The Planet of the <BLANK>"},
...
%{x: false, prompt: "Scientists say erosion, but we all know the Grand Canyon was actually made by <BLANK>"},
...
%{x: false, prompt: "On your wedding night, it would be horrible to find out that the person you married is <BLANK>"},
%{x: false, prompt: "The Skittles flavor that just missed the cut"},
%{x: false, prompt: "What FDR meant to say was \u201cWe have nothing to fear, but <BLANK>\u201d"},
...
%{x: false, prompt: "Something you shouldn\u2019t get your significant other for Valentine\u2019s Day"},
...
%{x: false, prompt: "What would you do if you were left alone in the White House for an hour?"},
%{x: false, prompt: "Come up with the name of book that would sell a million copies, immediately"},
...
%{x: false, prompt: "There\u2019s Gryffindor, Ravenclaw, Slytherin, and Hufflepuff, but what\u2019s the Hogwarts house few have ever heard of?"},
...
%{x: false, prompt: "Come up with a really bad football penalty that begins with \u201cIntentional\u201d"},
%{x: false, prompt: "You know you\u2019re in for a bad taxi ride when <BLANK>"},
%{x: false, prompt: "The terrible fate of the snowman Olaf in a director\u2019s cut of <i>Frozen<\/i>"},
...
%{x: false, prompt: "Usually, it\u2019s bacon, lettuce and tomato, but come up with a BLT you wouldn\u2019t want to eat"},
...
%{x: false, prompt: "If you were allowed to name someone else\u2019s baby any weird thing you wanted, what would you name it?"},
%{x: false, prompt: "A terrible name for a clown"},
%{x: false, prompt: "Miller Lite beer would make a lot of money if they came up with a beer called Miller Lite _____"},
%{x: false, prompt: "Okay... fine! What do YOU want to talk about then?!!!"},
%{x: false, prompt: "The Katy Perry Super Bowl halftime show would have been better with <BLANK>"},
%{x: false, prompt: "Your personal catchphrase if you were on one of those <i>Real Housewives<\/i> shows"},
...
%{x: false, prompt: "Write a newspaper headline that will really catch people\u2019s attention"},
...
%{x: false, prompt: "Come up with the name for a new TV show with the word \u201cSpanky\u201d in it"},
...
%{x: false, prompt: "The most presidential name you can think of (that isn\u2019t already the name of a president)"},
...
%{x: false, prompt: "The title you\u2019d come up with if you were writing the Olympics theme song"},
%{x: false, prompt: "Name the sequel to <i>Titanic<\/i> if there were one. <i>Titanic 2: <BLANK><\/i>"},
...
%{x: false, prompt: "Like chicken fingers or chicken poppers, a new appetizer name for your fun, theme restaurant: chicken _____"},
...
%{x: false, prompt: "If God has a sense of humor, he welcomes people to heaven by saying, \u201c<BLANK>\u201d"},
%{x: false, prompt: "Something that would not work well as a dip for tortilla chips"},
%{x: false, prompt: "Name a new movie starring a talking goat who is president of the United States"},
...
%{x: false, prompt: "Make up a word that describes the sound of farting into a bowl of mac & cheese"},
...
%{x: false, prompt: "The name of a toilet paper specifically designed for the Queen of England"},
%{x: false, prompt: "A terrible name for a 1930s gangster"},
%{x: false, prompt: "Something upsetting you could say to the cable guy as he installs your television service"},
...
%{x: false, prompt: "If a winning coach gets Gatorade dumped on his head, what should get dumped on the losing coach?"},
...
%{x: false, prompt: "Come up with a three-word sequel to the book \u201cEat, Pray, Love\u201d"},
%{x: false, prompt: "You wouldn\u2019t want to share a prison cell with someone named <BLANK>"},
...
%{x: true, prompt: "Most people think Julius Caesar said \u201cEt tu, Brute?\u201d when he got stabbed, but what he really said was <BLANK>"},
...
%{x: true, prompt: "A disturbing thing to hear your significant other say while sleep-talking"},
...
%{x: true, prompt: "Bad: You\u2019re lost in the woods. Worse: You\u2019re also completely naked. Worst: And you\u2019re also <BLANK>"},
%{x: true, prompt: "How you can tell you\u2019re drinking really cheap wine"},
%{x: true, prompt: "An obscure Surgeon General warning that most people don\u2019t know about: \u201cSmoking may cause <BLANK>\u201d"},
...
%{x: true, prompt: "A surprising new part of the field sobriety test requires you to <BLANK> to prove you\u2019re not drunk"},
%{x: true, prompt: "A name for a sexy turtle"},
%{x: true, prompt: "What Little Bo Peep would confess if she got really drunk"},
%{x: true, prompt: "The first thought that runs through your head when a lobster clamps onto your genitals"},
...
%{x: true, prompt: "What happens when you finally make eye contact with the crazy person on the subway"},
...
%{x: true, prompt: "Rename any famous work of literature so that it is ruined by the word \u201cbutt\u201d"},
...
%{x: true, prompt: "The worst children\u2019s board game would be \u201c<BLANK>, <BLANK> Hippos\u201d"},
%{x: true, prompt: "The worst thing to whisper during pillow talk"},
%{x: true, prompt: "Something that absolutely doesn\u2019t make you think of a penis on some level"},
...
%{x: true, prompt: "Something a talking doll probably should NOT say when you pull the string"},
%{x: true, prompt: "A kinky weird thing that does NOT happen in 50 Shades of Grey (as far as you know)"},
...
%{x: true, prompt: "A new, completely BS holiday that greeting card companies would make up to sell more cards"},
...
%{x: true, prompt: "The Seven Deadly Sins are lust, gluttony, greed, envy, pride, wrath, and sloth. The Eighth Deadly Sin is..."},
...
%{x: true, prompt: "The celebrity you\u2019d eat first if you were a cannibal, and the side dish you\u2019d eat them with"},
%{x: true, prompt: "Santa Claus would be a bigger badass if his sleigh were driven by eight tiny\u2026"},
%{x: true, prompt: "A good name for a restaurant that serves animals with the faces still on them "},
%{x: true, prompt: "A name for a board game designed to give children nightmares "},
%{x: true, prompt: "The worst person to narrate the audiobook of <i>Fifty Shades of Grey<\/i>"},
%{x: true, prompt: "A good name for an erotic bakery"},
%{x: true, prompt: "What the Statue of Liberty is hiding beneath that robe"},
%{x: true, prompt: "There\u2019s only one time that murder is acceptable and that is when <BLANK>"},
%{x: true, prompt: "Take any well-known restaurant and slightly change its name to something inappropriate"},
...
%{x: true, prompt: "Take any U.S. president\u2019s name and turn it into something inappropriate"},
...
%{x: true, prompt: "Invent a family-friendly replacement word that you could say instead of an actual curse word"},
...
%{x: true, prompt: "If we can\u2019t afford to bury or cremate you, what should we do with your body?"},
