defmodule YouBet.Scores do
  def update(scores, players, guesses, odds, bets, answer) do
    ...
    new_scores = Enum.reduce(payouts, scores, fn {id, %{delta: delta}}, s ->
  ...
  defp get_payouts(players, guesses, odds, bets, answer) do
    ...
    players
    |> Enum.map(fn {id, _} ->
      {id, %{delta: get_delta_for_player(id, closest_guess, guesses, odds, bets), closest: guesses[id] == closest_guess}}
    end)
    |> Enum.into(%{})
  end
  ...
  def get_delta_for_player(id, closest_guess, guesses, odds, bets) do
    ...
    points_for_right_bet = if is_nil(right_bet), do: 0, else: (right_bet[:base_wager] + right_bet[:extra_wager]) * odds[closest_guess]
