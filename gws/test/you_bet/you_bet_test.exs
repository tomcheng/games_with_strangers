defmodule YouBetTest do
  ...
  test "sets stage to betting when all guesses are in", %{players: players} do
    state =
      players
      |> YouBet.initial_state(%{"rounds" => 7})
      |> YouBet.play("1", "guess", "30", "AAAA")
      |> YouBet.play("2", "guess", "20", "AAAA")
      |> YouBet.play("3", "guess", "10", "AAAA")
      |> YouBet.sanitize_state("1")
  ...
  test "changes to reveal stage when all bets are in and updates scores", %{players: players} do
    state =
      players
      |> YouBet.initial_state(%{"rounds" => 7})
      |> YouBet.play("1", "guess", "1", "AAAA")
      |> YouBet.play("2", "guess", "1", "AAAA")
      |> YouBet.play("3", "guess", "999999999999", "AAAA")
      |> YouBet.play(
        "1",
        "finalize_bets",
        [
  ...
  test "updates score when someone guesses less correctly", %{players: players} do
    state =
      players
      |> YouBet.initial_state(%{"rounds" => 7})
      |> YouBet.play("1", "guess", "999999999999", "AAAA")
      |> YouBet.play("2", "guess", "999999999999", "AAAA")
      |> YouBet.play("3", "guess", "999999999999", "AAAA")
      |> YouBet.play(
        "1",
        "finalize_bets",
  ...
  test "advances to next round", %{players: players} do
    state =
      ...
      |> YouBet.play("1", "advance_round", nil, "AAAA")
      |> YouBet.sanitize_state("1")
